...
// Go allows us to spawn external process. We do this
// when we need an external process accessible to a running Go process.
//
// Sometimes we just want to completely replace the current Go process
// with another (perhaps non-Go) one. To do this we’ll use
...
func main() {
	...
	// For our example we’ll exec ls. Go requires an absolute path
	// to the binary we want to execute, so we’ll use exec.LookPath
