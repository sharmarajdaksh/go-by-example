...
func main() {
	...
	// The io package provides some functions that may be
	// helpful for file reading. For example, reads like
	// the ones above can be more robustly implemented with ReadAtLeast.
	o3, err := f.Seek(6, 0)
	check(err)
	b3 := make([]byte, 2)
	// See docs
	...
	// The bufio package implements a buffered reader that may be
	// useful both for its efficiency with many small reads and
