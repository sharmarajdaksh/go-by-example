...
// Another alternaitve to mutexes is to use the built-in synchronization
// features of goroutines and channels to achieve the same result.
...
func main() {
	...
	// Here is the goroutine that owns the state, which is a
	// map as in the previous example but now private to the
	// stateful goroutine. This goroutine repeatedly selects
	...
	// and then sending a value on the response channel resp to
	// indicate success (and the desired value in the case of reads).
	go func() {
	...
	// the reads channel, and the receiving the result over the provided resp channel.
	for r := 0; r < 100; r++ {
		go func() {
			for {
				read := readOp{
...
// For this particular case the goroutine-based approach was a
// bit more involved than the mutex-based one. It might be useful
// in certain cases though, for example where you have other channels
// involved or when managing multiple such mutexes would be
// error-prone.
